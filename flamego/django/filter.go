...
func init() {
	_ = pongo2.RegisterFilter("contain", filterContain)
	_ = pongo2.RegisterFilter("trim", filterTrim)
	_ = pongo2.RegisterFilter("trimLeft", filterTrimLeft)
	_ = pongo2.RegisterFilter("trimRight", filterTrimRight)
	_ = pongo2.RegisterFilter("replace", filterReplace)
	_ = pongo2.RegisterFilter("list", filterList)
	_ = pongo2.RegisterFilter("fields", filterFields)
	_ = pongo2.RegisterFilter("count", filterCount)
	_ = pongo2.RegisterFilter("index", filterIndex)
	_ = pongo2.RegisterFilter("repeat", filterRepeat)
	_ = pongo2.RegisterFilter("json", filterJson)
	_ = pongo2.RegisterFilter("unixFormat", filterUnixFormat)
}
...
func filterJson(in *pongo2.Value, param *pongo2.Value) (*pongo2.Value, *pongo2.Error) {
	...
	result, ok := json.Marshal(in.Interface())
	if ok != nil {
		return nil, nil
	}
...
func filterUnixFormat(in *pongo2.Value, param *pongo2.Value) (*pongo2.Value, *pongo2.Error) {
	format := "Y-m-d H:i:s"

	if param != nil && param.IsString() {
		formatStr := param.String()
		if formatStr != "" {
			format = formatStr
		}
	}

	now := carbon.CreateFromTimestamp(int64(in.Integer()))
	return pongo2.AsValue(now.Format(format)), nil
}
